...
When two matching bids are submitted, the bid submitted on an earlier calendar day will take precedence.  If the bids are submitted on the same calendar day, then a tiebreak will be based on the rookie draft order.  Once a tie break is won, the winning owner goes to the last place in the order.
